...
var formWork;
//==创建DataGrid对象
var initToDoGrid = function(){
	$('#myToDoWorkList').datagrid({
		url:getCtxPath()+'/rest/work/myToDoWork?isWidget=true',
		method:'get',
		striped: true,
		singleSelect:true,
		columns:[[
	...
	$('#myToDoWorkList').parent().siblings('.datagrid-toolbar').find('.l-btn').last().css('float','right');
};

//==签收待办
function claimWork(){
	var workitemId;
	var row = $('#myToDoWorkList').datagrid('getSelected');
	if (row){
		if (row.workState != '0'){
...
//==刷新表格
function reloadWork(){
	$('#myToDoWorkList').datagrid('reload');
}

//==更多内容
function moreInfo(){
	var url = getCtxPath()+'/rest/work/myToDoWork/get/pre'
	top.menuCall("待办业务列表",url);
}
